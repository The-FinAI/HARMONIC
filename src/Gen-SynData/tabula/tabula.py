...
from tabula.tabula_dataset import TabulaDataset, TabulaDataset_FixSeed, TabulaDataCollator
...
class Tabula:
    ...
    def __init__(self, llm: str, experiment_dir: str = "trainer_tabula", epochs: int = 100,
                 ...
                 efficient_finetuning: str = "", **train_kwargs):
        """ Initializes Tabula.
    ...
    def fit(self, data: tp.Union[pd.DataFrame, np.ndarray], fixseed: tp.Union[bool], column_names: tp.Optional[tp.List[str]] = None,
        ...
        if not fixseed:
            ...
            tabula_ds = TabulaDataset_FixSeed.from_pandas(df)
        tabula_ds.set_tokenizer(self.tokenizer)

        # Set training hyperparameters
        logging.info("Create Tabula Trainer...")
        training_args = TrainingArguments(output_dir=self.experiment_dir,
    ...
    def sample(self, n_samples: int,
               ...
               temperature: float = 0.7, k: int = 10, max_length: int = 100, device: str = "cuda") -> pd.DataFrame:
    ...
    def tabula_sample(self, starting_prompts: tp.Union[str, list[str]], temperature: float = 0.7, max_length: int = 100,
        ...
        # 截取字符串
        result = [decoded_data[0][start_index:end_index]]
        df_gen = _convert_text_to_tabular_data(result, pd.DataFrame(columns=self.columns))
        return df_gen
